...
#define FPGA_REG_SPACE_SIZE          0x2000000
#define FPGA_MAX_TEMP_REG            0x0078
                                ...
                                char *buf)
{
    ...
    return sprintf(buf, "0x%8.8x\n", ioread32(fpga->base + FPGA_SCRATCH_ADDR) & 0xffffffff);
}
/**
                                ...
                                const char *buf, size_t count)
{
    ...
    iowrite32(data, fpga->base + FPGA_SCRATCH_ADDR);
    return count;
}
                                ...
                                char *buf)
{
    ...
    return sprintf(buf, "0x%8.8x\n", ioread32(fpga->base + FPGA_VERSION_ADDR) & 0xffffffff);
}
                         ...
                         loff_t off, size_t count)
{
    ...
    if ( off + count > FPGA_REG_SPACE_SIZE ) {
        return -EINVAL;
    }
    mutex_lock(&fpga->fpga_lock);
    while (i < count) {
...
static int cls_fpga_probe(struct platform_device *pdev)
{
    ...
    printk("FPGA version: 0x%x\n", ioread32(fpga->base + FPGA_VERSION_ADDR));
...
MODULE_DESCRIPTION("Celestica fpga access/control driver");
