...
def _open_sfp_lock():
    # The lock file is shared between the pmon container (root) and CLI tools
    # that may run as a non-root user (e.g. "show interfaces status", which
    # instantiates the platform Chassis and reads SFP presence). Create it
...
class ClsPddfSfp(PddfSfp):
    ...
    def write_ts(self, file_path):
        ...
        with _open_sfp_lock() as lock_fd:
    ...
    def read_ts(self, file_path):
        ...
        with _open_sfp_lock() as lock_fd:
            fcntl.flock(lock_fd, fcntl.LOCK_EX)
            # Read timestamp directly from file
            if os.path.exists(file_path):
                with open(file_path, "r") as fd:
                    ts_str = fd.read().strip()
            fcntl.flock(lock_fd, fcntl.LOCK_UN)
    ...
    def remove_ts(self, file_path):
        with _open_sfp_lock() as lock_fd:
            fcntl.flock(lock_fd, fcntl.LOCK_EX)
            # Remove file directly inside docker
            if os.path.exists(file_path):
                os.remove(file_path)
        fcntl.flock(lock_fd, fcntl.LOCK_UN)
